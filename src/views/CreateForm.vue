<template>
   <div class="flex flex-col lg:flex-row min-h-screen bg-gray-50 font-sans overflow-hidden">
      <!-- Left Side: Form Content -->
      <div class="flex-1 h-screen overflow-y-auto relative custom-scrollbar">
         <!-- Sticky Header Stepper -->
         <div class="sticky top-0 z-40 bg-gray-50/80 backdrop-blur-xl border-b border-gray-100 px-4 md:px-8 py-4 mb-8">
         ...
         <div class="max-w-5xl mx-auto px-4 md:px-8 pb-32">
      ...
      <div class="hidden lg:flex w-[480px] bg-slate-100 border-l border-gray-200 h-screen sticky top-0 flex-col items-center justify-center p-12">
         <div class="relative w-full max-w-[320px] aspect-[9/19] bg-dark rounded-[3.5rem] border-[14px] border-dark shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)] overflow-hidden">
            <!-- Notch -->
            ...
            <Transition name="fade">
               <div v-if="isPreviewLoading" class="absolute inset-0 bg-white flex flex-col items-center justify-center z-10">
                  <div class="w-12 h-12 border-4 border-mocha/20 border-t-mocha rounded-full animate-spin mb-4"></div>
                  <p class="text-[10px] font-black text-mocha uppercase tracking-widest">Sinkronisasi...</p>
               </div>
            </Transition>
         </div>

         <div class="mt-8 text-center">
            <div class="inline-flex items-center gap-2 px-3 py-1 bg-mocha/10 text-mocha rounded-full mb-3">
               ...
               <span class="text-[10px] font-black uppercase tracking-widest">Live Preview Mode</span>
            </div>
            <p class="text-xs text-slate-500 max-w-[280px] font-medium leading-relaxed">
               Setiap perubahan yang Anda buat di form kiri akan langsung terlihat di tampilan mobile ini secara real-time.
            </p>
         </div>
      </div>
   </div>
   ...
   <button @click="showMobilePreview = true" class="lg:hidden fixed bottom-24 right-6 z-50 w-14 h-14 bg-mocha text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-transform">
      ...
      <div v-if="showMobilePreview" class="lg:hidden fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-sm flex flex-col">
         <div class="flex items-center justify-between p-6 text-white">
            <div class="flex items-center gap-3">
               ...
               <span class="text-xs font-black uppercase tracking-widest">Live Preview</span>
            </div>
            <button @click="showMobilePreview = false" class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20">
               <i class="fa-solid fa-times"></i>
            </button>
         </div>
         
         <div class="flex-1 px-6 pb-6">
            <div class="w-full h-full bg-white rounded-[2.5rem] border-[8px] border-dark overflow-hidden relative">
               <iframe 
               ...
               <Transition name="fade">
                  <div v-if="isPreviewLoading" class="absolute inset-0 bg-white flex flex-col items-center justify-center">
                     <div class="w-12 h-12 border-4 border-mocha/20 border-t-mocha rounded-full animate-spin mb-4"></div>
                     <p class="text-[10px] font-black text-mocha uppercase tracking-widest">Memuat...</p>
                  </div>
               </Transition>
            </div>
         </div>
...
const previewUrl = computed(() => {
   const templateSlug = selectedTemplateRef.value.slug || 'dark-elegant'
...
const syncDataToPreview = (data) => {
   try {
      const rawPayload = {
         ...
         data: {
            title: data.title,
            template_slug: selectedTemplateRef.value.slug || 'dark-elegant',
...
const saveDraft = (data) => {
   ...
   try {
      const draftData = { formData: { ...data }, sections: { ...sections.value }, timestamp: Date.now() }
      Object.keys(draftData.formData).forEach(key => { if (key.toLowerCase().includes('file')) delete draftData.formData[key] })
...
onMounted(async () => {
   ...
   if (generalDraft && !route.params.id) {
      ...
      if (age < 24) {
         if (confirm("Anda memiliki draf yang belum tersimpan. Lanjutkan mengisi?")) {
...
async function uploadAllFiles() {
   ...
   for (const item of filesToUpload) {
      ...
      try { const res = await uploadFileApi(item.file); item.setter(res.fileUrl); uploadedCount++; uploadProgress.value.percentage = Math.round((uploadedCount / filesToUpload.length) * 100) }
      catch (err) { uploadProgress.value.show = false; throw err }
   }
...
async function saveAndPreview() {
   ...
   if (!selectedTemplateRef.value.id && !route.params.id) { toast.error("Template belum dipilih. Mohon ulangi dari awal."); router.push('/create'); return }
