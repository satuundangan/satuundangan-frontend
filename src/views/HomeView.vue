<template>
  <div class="bg-ivory min-h-screen font-sans text-dark scroll-smooth">
    ...
    <section id="templates" class="section bg-white scroll-mt-20">
      <div class="max-w-6xl mx-auto px-6">
        ...
        <div v-else class="grid md:grid-cols-3 gap-10">
          <div v-for="(item) in filteredTemplates.slice(0, 9)" :key="item.id" @click="selectTemplate(item.id); showModal = true;"
            ...
            <div class="relative overflow-hidden h-64 bg-gray-100">
              <img :src="item.previewUrl || 'https://via.placeholder.com/400x300?text=No+Preview'" :alt="item.name" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
              
              <!-- Hover Overlay -->
              <div class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                <span class="bg-white/90 text-mocha px-4 py-2 rounded-full font-bold text-sm shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 flex items-center gap-2">
                   👁️ Lihat Detail
                </span>
              </div>
              
              <!-- Badge Premium -->
              <div v-if="item.tags && item.tags.includes('premium')" class="absolute top-3 right-3 bg-gradient-to-r from-amber-400 to-amber-600 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-md tracking-wider uppercase">
                Premium
              </div>
            </div>
            ...
            <div class="p-5">
              ...
              <div class="flex flex-wrap gap-2">
                 <span v-for="tag in item.tags?.slice(0,3)" :key="tag" class="text-[10px] bg-sage/10 text-sage font-medium px-2 py-0.5 rounded-md">
                   #{{ tag }}
                 </span>
              </div>
            </div>
         ...
         <div class="text-center mt-12">
          <button @click="showModal = true" class="btn-outline px-8 rounded-full hover:shadow-lg transition-all">Lihat Semua Template</button>
        </div>
      </div>
    </section>
    ...
    <section id="testimonials" class="section bg-ivory relative overflow-hidden">
      <!-- Decor -->
      <div class="absolute top-0 left-0 w-96 h-96 bg-sage/5 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2"></div>
      <div class="absolute bottom-0 right-0 w-96 h-96 bg-mocha/5 rounded-full blur-3xl translate-x-1/2 translate-y-1/2"></div>
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <div v-for="(item, i) in testimonials" :key="i"
            ...
            <div class="flex items-center gap-4 border-t border-gray-100 pt-4">
              <img :src="item.avatar" alt="avatar" class="w-12 h-12 rounded-full object-cover ring-2 ring-sage/30 p-0.5" />
              <div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center px-4" @click.self="showModal = false">
      <div class="bg-white w-full max-w-6xl h-[90vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-scale-up">
        ...
        <div class="flex-1 flex flex-col min-w-0 bg-white relative">
           ...
           <div class="p-5 border-b border-gray-200 flex justify-between items-center bg-white z-10 shadow-sm">
              ...
              <button @click="showModal = false" class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-red-50 hover:text-red-500 transition">
                 &times;
              </button>
           </div>
           ...
           <div class="flex-1 overflow-y-auto p-6 bg-gray-50/50 custom-scrollbar">
              ...
              <div v-else-if="filteredTemplates.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
                 <div v-for="(item) in filteredTemplates" :key="item.id" :ref="el => templateRefs[item.id] = el"
                    ...
                    <div class="relative h-44 overflow-hidden bg-gray-200">
                       <img :src="item.previewUrl || 'https://via.placeholder.com/400x300'" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"/>
                       <div v-if="selectedTemplate === item.id" class="absolute inset-0 bg-mocha/40 backdrop-blur-[1px] flex items-center justify-center animate-fade-in">
                          <div class="bg-white rounded-full p-2 shadow-lg scale-110">
                             <svg class="w-6 h-6 text-mocha" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                          </div>
                       </div>
                    </div>
                    <div class="p-4 flex flex-col flex-1">
                       <div class="flex justify-between items-start mb-2">
                          <h4 class="font-bold text-dark text-sm line-clamp-1" :class="selectedTemplate === item.id ? 'text-mocha' : ''">{{ item.name }}</h4>
                       </div>
              ...
              <div v-else class="h-full flex flex-col items-center justify-center text-center text-gray-400 space-y-4">
                <div class="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center text-3xl grayscale opacity-50">📦</div>
                <div>
           ...
           <!-- Footer Modal -->
           <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200 bg-white flex justify-between items-center z-10 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
              <div class="hidden sm:block">
                 <p v-if="selectedTemplate" class="text-sm text-mocha font-medium">
                    Template terpilih: <span class="font-bold">{{ filteredTemplates.find(t => t.id === selectedTemplate)?.name }}</span>
                 </p>
                 <p v-else class="text-sm text-gray-400">Silakan pilih template.</p>
              </div>
              <div class="flex gap-3 w-full sm:w-auto">
                 <button @click="showModal = false" class="flex-1 sm:flex-none px-6 py-2.5 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm font-semibold transition">Batal</button>
                 <button 
