          ...
          <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div class="flex items-center gap-4 mb-6 pb-6 border-b border-gray-50">
                <div
                  class="h-16 w-16 rounded-2xl bg-mocha/10 flex items-center justify-center text-mocha text-2xl font-bold border-2 border-white shadow-sm"
                >
                  {{ userName.charAt(0) }}
                </div>
                <div>
              ...
              <form @submit.prevent="handleUpdateProfile" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      type="email"
                      disabled
                      class="w-full border border-gray-100 rounded-xl p-3 bg-gray-100 text-gray-400 cursor-not-allowed text-sm"
                ...
                <div class="flex justify-end pt-2">
                  <button
                    ...
                    class="w-full md:w-auto px-8 py-3 bg-mocha text-white rounded-xl text-sm font-bold shadow-lg shadow-mocha/20 hover:bg-dark transition-all disabled:opacity-50"
                  >
              ...
              <form @submit.prevent="handleUpdatePassword" class="space-y-4">
                ...
                <div class="flex justify-end pt-2">
                  <button
                    ...
                    class="w-full md:w-auto px-8 py-3 bg-white border border-mocha text-mocha rounded-xl text-sm font-bold hover:bg-mocha/5 transition-all disabled:opacity-50"
                  >
            ...
            <div class="bg-mocha rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
              <div class="relative z-10">
                ...
                <p class="text-xs text-white/80 mb-4 leading-relaxed">
                  Punya kendala dengan akunmu? Tim support kami siap membantumu kapan saja.
                </p>
                <a
                  href="https://wa.me/628123456789"
                  target="_blank"
                  class="inline-flex items-center gap-2 bg-white text-mocha px-4 py-2 rounded-xl text-xs font-bold shadow-md"
                >
                  <i class="fa-brands fa-whatsapp text-sm"></i> Chat Support
                </a>
              </div>
              <i
                class="fa-solid fa-headset absolute -right-6 -bottom-6 text-white/10 text-8xl rotate-12"
              ></i>
            </div>
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <h3 class="font-bold text-dark text-xs uppercase tracking-widest mb-4">
                Informasi Akun
              </h3>
              <div class="space-y-3">
                <div class="flex justify-between items-center py-2 border-b border-gray-50">
                  ...
                  <span
                    class="text-[10px] font-bold px-2 py-0.5 bg-green-50 text-green-600 rounded-md border border-green-100 uppercase"
                    >Aktif</span
                  >
                </div>
                <div class="flex justify-between items-start gap-3 py-2 border-b border-gray-50">
                  ...
                  <div class="text-right">
                    <span
                      class="text-[10px] font-bold px-2 py-0.5 rounded-md border uppercase"
                      :class="
                        isEmailVerified
                    ...
                    <button
                      ...
                      type="button"
                      class="block ml-auto mt-2 text-[11px] font-bold text-mocha hover:underline disabled:opacity-50"
                      :disabled="resendingVerification"
                      @click="handleResendVerification"
                    >
                      {{ resendingVerification ? 'Mengirim...' : 'Kirim Ulang Link' }}
                    </button>
                    <a
                      v-if="devVerificationUrl"
                      :href="devVerificationUrl"
                      class="block mt-2 max-w-[180px] break-all text-[10px] font-semibold text-mocha hover:underline"
                    >
                      Development link
                    </a>
                  </div>
...
const isEmailVerified = computed(
  () => auth.user?.provider === 'google' || Boolean(auth.user?.emailVerifiedAt),
)
...
onMounted(async () => {
  if (route.query.email_verified === 'success') {
    await auth.fetchProfile()
    toast.success('Email berhasil diverifikasi')
    router.replace({ query: { ...route.query, email_verified: undefined } })
  } else if (route.query.email_verified === 'failed') {
    toast.error('Link verifikasi email tidak valid atau sudah kedaluwarsa')
    router.replace({ query: { ...route.query, email_verified: undefined } })
  }
})
...
async function handleResendVerification() {
  ...
  try {
    ...
    toast.success(res.message || 'Link verifikasi email dikirim')
    await auth.fetchProfile()
  } catch (err) {
    toast.error(err.message || 'Gagal mengirim link verifikasi')
  } finally {
