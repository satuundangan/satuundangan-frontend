          ...
          >
            ...
            <div class="relative z-10">
              ...
              <p class="text-white/80 text-sm leading-relaxed">
                Buat undangan pernikahan digital impianmu dengan fitur premium dan desain eksklusif
                hanya di SatuUndangan.
              </p>
            </div>

            <div class="relative z-10 text-xs text-white/60">© 2026 SatuUndangan.id</div>
          </div>
          ...
          <div class="flex-1 p-8 md:p-12 flex flex-col justify-center overflow-y-auto">
            <div class="max-w-sm mx-auto w-full">
              ...
              <form @submit.prevent="handleSubmit" class="space-y-4">
                ...
                <div
                  v-if="devResetUrl"
                  class="rounded-xl border border-mocha/10 bg-mocha/5 p-3 text-xs leading-relaxed text-gray-600"
                >
                  <p class="font-bold text-mocha">Development reset link:</p>
                  <router-link
                    :to="devResetRoute"
                    class="break-all font-semibold text-mocha hover:underline"
                    @click="emit('close')"
                  >
                    {{ devResetUrl }}
                  </router-link>
                ...
                <div
                  ...
                  class="rounded-xl border border-mocha/10 bg-mocha/5 p-3 text-xs leading-relaxed text-gray-600"
                >
                  <p class="font-bold text-mocha">Development verification link:</p>
                  <a
              ...
              <div class="mt-8 space-y-3 text-center text-sm text-muted">
                <p v-if="authMode === 'login'">
                  <a
                    href="#"
                    class="font-bold text-mocha hover:underline"
                    @click.prevent="setAuthMode('forgot')"
                  >
                    Lupa password?
                  </a>
                </p>
...
const mobileSubtitle = computed(() => {
  if (props.authMode === 'forgot')
    return 'Masukkan email akun untuk menerima instruksi reset password.'
...
const handleRegister = async () => {
  ...
  try {
    ...
    if (!devVerificationUrl.value) emit('close')
  } catch (e) {
