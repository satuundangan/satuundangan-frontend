<template>
  <div class="relative min-h-screen bg-[#fff8fa] overflow-hidden font-sans no-scrollbar text-[#5e3a45] selection:bg-[#d6336c]/30">
    ...
    <transition name="fade">
      <img v-if="isOpened" src="/assets/images/sakura-bg.png" alt=""
        aria-hidden="true"
    ...
    <img v-if="isOpened" src="/assets/images/sakura-falling-petals.png" alt=""
      aria-hidden="true"
    ...
    <!-- Music Control -->
    <MusicControl v-if="data.musicChoice" :src="getMusicUrl(data.musicChoice)" :audioStart="data.audioStart" :audioEnd="data.audioEnd" primaryColor="#fff5f7" accentColor="#d6336c" />
    ...
    <nav v-if="isOpened"
      ...
      <div class="flex items-center justify-center gap-8 px-6 py-3 mx-auto min-w-max">
        <button v-for="item in navItems" :key="item.id" @click="scrollToSection(item.id)"
          ...
          :class="activeSection === item.id ? 'text-[#ffb6c1] scale-110' : 'text-white/40 hover:text-[#ffb6c1]'">
    ...
    <transition name="fade-welcome">
      <div v-if="!isOpened"
        ...
        <div class="relative z-10 space-y-10 animate-fade-in-up max-w-md mx-auto">
          <div class="space-y-3">
            <span class="text-[10px] tracking-[0.4em] uppercase text-[#d6336c] font-bold">The Wedding Of</span>
            <div class="flex flex-col items-center">
              <h2 class="text-5xl md:text-7xl font-shippori text-[#5e3a45]">{{ data.groomName }}</h2>
              <span class="text-2xl text-[#d6336c] my-1 font-shippori">桜</span>
              <h2 class="text-5xl md:text-7xl font-shippori text-[#5e3a45]">{{ data.brideName }}</h2>
            </div>
          ...
          <div class="space-y-2">
            <p class="text-[10px] uppercase tracking-widest text-[#c98ba0]">Kepada Yth. Bapak/Ibu/Saudara/i</p>
          ...
          <button @click="openInvitation" class="group flex flex-col items-center gap-3 mx-auto">
            <img src="/assets/images/sakura-envelope-icon.png" alt="Buka Undangan"
            ...
            <span class="text-[10px] uppercase tracking-[0.4em] font-bold text-[#d6336c]">Buka Undangan</span>
          </button>
        </div>
      </div>
    </transition>
    ...
    <!-- MAIN CONTENT -->
    <div v-if="isOpened" id="main-content" class="relative z-30 opacity-0 transition-opacity duration-1000 h-screen overflow-y-auto no-scrollbar scroll-smooth">
      ...
      <section id="home" class="min-h-screen flex flex-col items-center justify-center text-center px-6 relative">
        <div class="space-y-8 max-w-2xl mx-auto" v-observe>
          ...
          <h1 class="text-5xl md:text-8xl font-shippori leading-tight text-[#5e3a45]">
            {{ data.groomName }} <br> <span class="text-3xl text-[#d6336c] font-shippori">&</span> <br> {{ data.brideName }}
          </h1>
          ...
          <div class="flex justify-center gap-4 md:gap-6 mt-16">
            <div v-for="(val, label) in countdown" :key="label" class="flex flex-col items-center bg-white/60 backdrop-blur-sm rounded-2xl px-4 py-3 shadow-sm min-w-[64px]">
              <div class="text-3xl md:text-5xl font-light text-[#5e3a45]">{{ val }}</div>
              <div class="text-[8px] uppercase tracking-widest text-[#d6336c] font-bold">{{ label }}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="py-32 px-6 bg-white/40" v-observe>
        <div class="max-w-2xl mx-auto text-center border-l-4 border-[#ffb6c1] pl-8">
          ...
          <p class="text-xs font-bold text-[#d6336c] tracking-[0.2em] uppercase">— {{ data.quoteSource }}</p>
        </div>
      </section>

      <!-- LOVE STORY -->
      <section id="story" v-if="isSectionEnabled('love-story') && (data.loveStory?.length || isPreviewMode)" class="py-32 px-6 bg-[#fff8fa]">
        <div class="max-w-4xl mx-auto space-y-24">
          <div class="text-center space-y-4" v-observe>
            <span class="text-[10px] uppercase tracking-[0.4em] text-[#d6336c] font-bold">Hanami Story</span>
            <h2 class="text-3xl md:text-5xl font-shippori italic text-[#5e3a45]">Kisah Kasih Kami</h2>
               ...
               <div class="grid md:grid-cols-2 gap-12 items-center">
                  <div :class="idx % 2 === 0 ? '' : 'md:order-last'">
                     <div class="rounded-[2rem] overflow-hidden shadow-lg aspect-square">
                        ...
                        <div v-else class="w-full h-full bg-[#ffeef3] flex items-center justify-center"><i class="fa-solid fa-heart text-[#d6336c]/20 text-4xl"></i></div>
                     </div>
                  </div>
                  <div class="space-y-4" :class="idx % 2 === 0 ? 'text-left' : 'text-left md:text-right'">
                     <span class="text-[#d6336c] font-bold text-xs uppercase tracking-[0.2em]">{{ story.date }}</span>
                     <h3 class="text-2xl md:text-3xl font-shippori italic text-[#5e3a45]">{{ story.title }}</h3>
                     <p class="text-[#a9788a] text-sm md:text-base leading-relaxed font-light">{{ story.description }}</p>
                  </div>
               </div>
            </div>
          </div>
      ...
      <section id="couple" v-if="isSectionEnabled('couple')" class="py-32 px-6">
        <div class="max-w-5xl mx-auto">
          <div class="text-center space-y-4 mb-24" v-observe>
            <span class="text-[10px] uppercase tracking-[0.4em] text-[#d6336c] font-bold">Mempelai</span>
          ...
          <div class="grid md:grid-cols-2 gap-24 items-start">
            ...
            <div class="space-y-8" v-observe>
              <div class="relative">
                <div class="absolute -top-6 -left-6 w-32 h-32 bg-[#ffb6c1]/30 rounded-full -z-10 animate-pulse"></div>
                <div class="rounded-[3rem] rounded-tr-none overflow-hidden shadow-xl aspect-[4/5] bg-[#ffeef3]">
                  <img :src="data.groomPhotoUrl || 'https://via.placeholder.com/400x600'" class="w-full h-full object-cover" />
                </div>
              </div>
              <div class="space-y-2">
                <h3 class="text-3xl md:text-4xl font-shippori italic text-[#5e3a45]">{{ data.groomName }}</h3>
                <p class="text-xs uppercase tracking-widest text-[#d6336c] font-bold">Mempelai Pria</p>
                <p class="text-sm text-[#a9788a] leading-relaxed pt-2">Putra dari {{ data.parents?.groomParents || 'Bpk. & Ibu' }}</p>
                <a v-if="data.socialMediaGroom?.instagram" :href="formatInstagramUrl(data.socialMediaGroom.instagram)" target="_blank" class="inline-block mt-4 text-[#5e3a45] hover:text-[#d6336c] transition-colors">
            ...
            <div class="space-y-8 md:mt-32" v-observe>
              <div class="relative">
                <div class="absolute -bottom-6 -right-6 w-32 h-32 bg-[#d6336c]/15 rounded-full -z-10 animate-pulse"></div>
                <div class="rounded-[3rem] rounded-bl-none overflow-hidden shadow-xl aspect-[4/5] bg-[#ffeef3]">
                  <img :src="data.bridePhotoUrl || 'https://via.placeholder.com/400x600'" class="w-full h-full object-cover" />
                </div>
              </div>
              <div class="space-y-2 md:text-right">
                <h3 class="text-3xl md:text-4xl font-shippori italic text-[#5e3a45]">{{ data.brideName }}</h3>
                <p class="text-xs uppercase tracking-widest text-[#d6336c] font-bold">Mempelai Wanita</p>
                <p class="text-sm text-[#a9788a] leading-relaxed pt-2">Putri dari {{ data.parents?.brideParents || 'Bpk. & Ibu' }}</p>
                <a v-if="data.socialMediaBrides?.instagram" :href="formatInstagramUrl(data.socialMediaBrides.instagram)" target="_blank" class="inline-block mt-4 text-[#5e3a45] hover:text-[#d6336c] transition-colors">
                  <i class="fa-brands fa-instagram text-2xl"></i>
                </a>
              </div>
            </div>
      ...
      <!-- EXTENDED FAMILY -->
      <section v-if="isSectionEnabled('extended-family') && data.extendedFamily?.length" class="py-32 px-6 bg-white/40 text-center border-y border-[#ffb6c1]/30">
         <div class="max-w-4xl mx-auto space-y-12" v-observe>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
               <div v-for="(person, idx) in data.extendedFamily" :key="idx" class="p-6 bg-[#fff8fa] rounded-[2rem] text-sm text-[#5e3a45] font-medium tracking-wide">
                  {{ person }}
               </div>
            </div>
         </div>
      ...
      <!-- EVENTS -->
      <section id="event" v-if="isSectionEnabled('event')" class="py-32 px-6 bg-[#4a2530] text-[#fbe4ec] rounded-[4rem] mx-4 md:mx-10 my-10">
        <div class="max-w-4xl mx-auto text-center space-y-20">
          ...
          <div v-if="isSectionEnabled('live-streaming') && data.liveStreamingUrl" class="mt-20 text-center" v-observe>
             <a :href="data.liveStreamingUrl" target="_blank" class="inline-flex items-center gap-4 px-12 py-5 bg-[#ffb6c1] text-[#4a2530] rounded-full hover:bg-white transition-all shadow-2xl hover:-translate-y-1">
                <i class="fa-solid fa-video animate-pulse"></i>
                <span class="text-xs font-black uppercase tracking-[0.3em]">Tonton Live Streaming</span>
             </a>
          </div>
      ...
      <!-- GALLERY -->
      <section id="gallery" v-if="isSectionEnabled('gallery') && galleryImages.length" class="py-32 px-4 bg-[#fff8fa]">
        <div class="max-w-6xl mx-auto">
          <div class="flex flex-col items-center mb-20 space-y-4 text-center">
            <span class="text-[10px] uppercase tracking-[0.4em] text-[#d6336c] font-bold" v-observe>Gallery</span>
            <h2 class="text-4xl md:text-6xl font-shippori italic text-[#5e3a45]" v-observe>Galeri Momen</h2>
      ...
      <section id="rsvp" v-if="isSectionEnabled('rsvp')" class="py-32 px-6">
        <div class="max-w-xl mx-auto bg-white rounded-[3rem] p-10 md:p-16 shadow-sm border border-[#ffb6c1]/30" v-observe>
          <div class="text-center mb-12">
            <h2 class="text-4xl font-shippori italic text-[#5e3a45]">RSVP</h2>
            <p class="text-[#d6336c] text-[10px] uppercase tracking-widest font-bold mt-2">Konfirmasi Kehadiran</p>
          </div>

          <form @submit.prevent="submitRSVP" class="space-y-8">
            <div class="space-y-2">
              <label class="text-[10px] uppercase tracking-widest text-[#c98ba0] font-bold ml-2">Nama Anda</label>
            ...
            <div class="grid grid-cols-3 gap-3">
              <label v-for="opt in [{v:'hadir',l:'Hadir'},{v:'tidak',l:'Tidak Hadir'},{v:'ragu',l:'Ragu-ragu'}]" :key="opt.v"
                ...
                :class="rsvp.attendance === opt.v ? 'bg-[#4a2530] border-[#4a2530] text-white shadow-lg' : 'bg-transparent border-[#fff8fa] text-[#c98ba0] hover:border-[#ffb6c1]/50'">
            ...
            <textarea v-model="rsvp.message" rows="4" placeholder="Doa & Ucapan Anda"
              class="w-full bg-[#fff8fa] border-0 rounded-2xl px-6 py-4 text-[#5e3a45] focus:ring-2 focus:ring-[#ffb6c1] outline-none placeholder-[#e0c3cc]"></textarea>
      ...
      <!-- VIDEO PREWEDDING -->
      <section v-if="data.videoPrewedding && isSectionEnabled('video')" class="py-32 px-6 bg-[#fff8fa]">
        <div class="max-w-4xl mx-auto text-center">
          <h2 class="text-3xl font-shippori italic text-[#5e3a45] mb-10" v-observe>Video Prewedding</h2>
          <div class="relative w-full aspect-video rounded-3xl overflow-hidden shadow-2xl border border-[#ffb6c1]/30">
      ...
      <section v-if="data.menu?.items?.length && isSectionEnabled('menu')" class="py-32 px-6 bg-white/40">
        <div class="max-w-3xl mx-auto text-center">
          <h2 class="text-3xl font-shippori italic text-[#5e3a45] mb-10" v-observe>{{ data.menu.title || 'Menu Hidangan' }}</h2>
          <div class="grid gap-4 md:grid-cols-2">
            <div v-for="(item, idx) in data.menu.items" :key="idx"
              class="bg-[#fff8fa] p-6 rounded-[2rem] border border-[#ffb6c1]/30 text-left" v-observe>
      ...
      <!-- GIFT -->
      <section v-if="isSectionEnabled('gift') && (data.bankAccounts?.length || data.eWalletLink?.length)" id="gift" class="py-32 px-6 text-center bg-[#fff8fa]">
        <h2 class="text-3xl font-shippori italic text-[#5e3a45] mb-4" v-observe>Hadiah Pernikahan</h2>
        <p class="text-[#a9788a] mb-16 max-w-md mx-auto text-sm leading-relaxed">Doa restu Anda adalah karunia terindah bagi kami. Namun jika ingin memberi tanda kasih, dapat melalui:</p>

        <div class="flex flex-wrap justify-center gap-8 max-w-5xl mx-auto">
          <div v-for="(bank, idx) in data.bankAccounts" :key="idx"
            class="bg-white p-10 rounded-[3rem] w-full sm:w-[320px] shadow-sm hover:shadow-xl transition-all border border-[#ffb6c1]/20" v-observe>
            <div class="h-12 flex items-center justify-center mb-8 text-[#d6336c] font-bold text-2xl tracking-widest uppercase font-shippori">
              {{ bank.bankName }}
            </div>
            <p class="text-xl text-[#5e3a45] font-medium mb-2 tracking-widest">{{ bank.accountNumber }}</p>
            <p class="text-[10px] text-[#c98ba0] mb-8 uppercase tracking-[0.2em] font-bold">a.n {{ bank.accountName }}</p>
      ...
      <footer class="py-32 bg-white text-center rounded-t-[4rem]">
        ...
        <h2 class="font-shippori italic text-5xl text-[#5e3a45] mb-4">{{ data.groomName }} & {{ data.brideName }}</h2>
        <p class="text-[#d6336c] text-[10px] tracking-[0.5em] uppercase font-bold mb-12">Sampai Jumpa di Hari Bahagia Kami</p>
        <p class="text-[#e0c3cc] text-[8px] tracking-[0.4em] uppercase font-bold italic">Created by SatuUndangan.id</p>
      </footer>
...
const SakuraFlower = {
  render() {
    ...
    return h(
      ...
      [0, 72, 144, 216, 288].map(petal).concat([h('circle', { cx: 50, cy: 50, r: 6, class: 'opacity-70' })]),
    )
  },
}
...
async function openInvitation() {
  ...
  setTimeout(() => { burstActive.value = false }, 600)
  initScrollSpy()
}
...
function initData() {
  ...
  const targetDate = data.value.resepsiLocation?.dateTime || data.value.akadLocation?.dateTime || data.value.dateTime
  if (targetDate) {
...
.fade-welcome-enter-active, .fade-welcome-leave-active { transition: opacity 1s ease; }
.fade-welcome-enter-from, .fade-welcome-leave-to { opacity: 0; }
