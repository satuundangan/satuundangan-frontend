<template>
  <div class="relative min-h-screen bg-black overflow-hidden font-sans no-scrollbar font-montserrat text-[#f0f0f0]">
    ...
    <!-- Bottom Navigation -->
    <nav v-if="!showWelcome" class="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 bg-black/60 backdrop-blur-xl border border-white/10 rounded-full px-6 py-3 flex gap-6 shadow-2xl animate-slide-up w-max max-w-[90%] overflow-x-auto no-scrollbar">
       <a href="#home" class="text-white/60 hover:text-[#d6b18a] transition-colors flex flex-col items-center gap-1 min-w-[40px]">
          <i class="fa-solid fa-house text-lg"></i>
          <span class="text-[10px]">Home</span>
       </a>
       <a href="#couple" class="text-white/60 hover:text-[#d6b18a] transition-colors flex flex-col items-center gap-1 min-w-[40px]">
          <i class="fa-solid fa-heart text-lg"></i>
          <span class="text-[10px]">Couple</span>
       </a>
       <a href="#event" class="text-white/60 hover:text-[#d6b18a] transition-colors flex flex-col items-center gap-1 min-w-[40px]">
          <i class="fa-solid fa-calendar-check text-lg"></i>
          <span class="text-[10px]">Event</span>
       </a>
       <a href="#rsvp" class="text-white/60 hover:text-[#d6b18a] transition-colors flex flex-col items-center gap-1 min-w-[40px]">
    ...
    <transition name="fade">
      <div v-if="showWelcome"
        ...
        <div class="relative z-10 space-y-6 animate-fade-in-up w-full">
          <div class="w-16 h-16 md:w-20 md:h-20 mx-auto border border-[#d6b18a] rounded-full flex items-center justify-center text-2xl md:text-3xl mb-4">
             🌿
          </div>
    ...
    <div v-if="!showWelcome" id="main-content" class="opacity-0 transition-opacity duration-1000 h-full overflow-y-auto no-scrollbar scroll-smooth">
      
      <!-- HERO -->
      <section id="home" class="relative min-h-screen flex flex-col items-center justify-center text-center px-6 bg-fixed bg-cover bg-center"
        :style="{ backgroundImage: `url(${backgroundUrl})` }">
        ...
        <div class="relative z-10 space-y-6" v-observe>
           ...
           <p class="text-base md:text-lg text-gray-200 font-light tracking-wide">{{ formatDate(data.akadLocation?.dateTime) }}</p>
      ...
      <section id="couple" class="py-20 md:py-24 px-6 bg-[#1a1a1a]">
         <div class="max-w-6xl mx-auto">
            <h2 class="text-3xl md:text-4xl font-alex text-center text-[#d6b18a] mb-12 md:mb-16" v-observe>The Happy Couple</h2>
            
            <div class="grid md:grid-cols-2 gap-12 items-center">
               <!-- Groom -->
               <div class="relative group" v-observe>
                  <div class="absolute inset-0 bg-[#d6b18a] rotate-3 rounded-[2rem] md:rounded-[3rem] opacity-20 group-hover:rotate-6 transition-transform duration-500"></div>
                  <div class="relative bg-black rounded-[2rem] md:rounded-[3rem] overflow-hidden border border-[#d6b18a]/30 shadow-2xl">
                     <img :src="data.groomPhotoUrl || 'https://via.placeholder.com/400x600'" class="w-full h-[400px] md:h-[500px] object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                     <div class="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black to-transparent p-6 md:p-8 text-center">
                        <h3 class="text-2xl md:text-3xl font-alex text-[#d6b18a]">{{ data.groomName }}</h3>
                        <p class="text-xs md:text-sm text-gray-400 mt-2">Putra dari Bpk. {{ getParentName(data.parents?.groomParents, 0) }} & Ibu {{ getParentName(data.parents?.groomParents, 1) }}</p>
                        <a v-if="data.socialMediaGroom?.instagram" :href="formatInstagramUrl(data.socialMediaGroom.instagram)" target="_blank" class="inline-block mt-4 text-[#d6b18a] hover:text-white transition-colors">
               ...
               <div class="relative group mt-8 md:mt-24" v-observe>
                  <div class="absolute inset-0 bg-[#d6b18a] -rotate-3 rounded-[2rem] md:rounded-[3rem] opacity-20 group-hover:-rotate-6 transition-transform duration-500"></div>
                  <div class="relative bg-black rounded-[2rem] md:rounded-[3rem] overflow-hidden border border-[#d6b18a]/30 shadow-2xl">
                     <img :src="data.bridePhotoUrl || 'https://via.placeholder.com/400x600'" class="w-full h-[400px] md:h-[500px] object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                     <div class="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black to-transparent p-6 md:p-8 text-center">
                        <h3 class="text-2xl md:text-3xl font-alex text-[#d6b18a]">{{ data.brideName }}</h3>
                        <p class="text-xs md:text-sm text-gray-400 mt-2">Putri dari Bpk. {{ getParentName(data.parents?.brideParents, 0) }} & Ibu {{ getParentName(data.parents?.brideParents, 1) }}</p>
                        <a v-if="data.socialMediaBrides?.instagram" :href="formatInstagramUrl(data.socialMediaBrides.instagram)" target="_blank" class="inline-block mt-4 text-[#d6b18a] hover:text-white transition-colors">
                           <i class="fa-brands fa-instagram text-xl"></i>
                        </a>
                     </div>
                  </div>
      ...
      <section id="event" class="py-20 md:py-24 px-6 relative overflow-hidden">
         <!-- Parallax Background -->
         <div class="absolute inset-0 bg-fixed bg-cover bg-center opacity-20" :style="{ backgroundImage: `url(${backgroundUrl})` }"></div>
         ...
         <div class="max-w-4xl mx-auto relative z-10 text-center">
            ...
            <p class="text-sm md:text-base text-gray-300 mb-8 md:mb-12">Kami menantikan kehadiran Anda di hari bahagia kami.</p>

            <div class="grid gap-8">
               <!-- Akad -->
               <div class="bg-black/60 backdrop-blur-md border border-[#d6b18a]/20 p-6 md:p-8 rounded-3xl relative overflow-hidden group hover:border-[#d6b18a]/50 transition-all" v-observe>
                  <div class="absolute -right-10 -top-10 text-8xl md:text-9xl text-[#d6b18a]/5 group-hover:rotate-12 transition-transform">💍</div>
                  ...
                  <a :href="data.akadLocation?.mapUrl" target="_blank" class="inline-flex items-center justify-center px-6 py-2.5 border border-[#d6b18a] text-[#d6b18a] rounded-lg hover:bg-[#d6b18a] hover:text-black transition-all text-xs md:text-sm uppercase tracking-wide font-medium">
                     <i class="fa-solid fa-location-dot mr-2"></i> Google Maps
                  </a>
               </div>

               <!-- Resepsi -->
               <div v-if="!data.mergeEvents" class="bg-black/60 backdrop-blur-md border border-[#d6b18a]/20 p-6 md:p-8 rounded-3xl relative overflow-hidden group hover:border-[#d6b18a]/50 transition-all" v-observe>
                  <div class="absolute -left-10 -top-10 text-8xl md:text-9xl text-[#d6b18a]/5 group-hover:-rotate-12 transition-transform">🥂</div>
                  ...
                  <p class="text-gray-300 text-xs md:text-sm mb-6 max-w-md mx-auto">{{ data.resepsiLocation?.description }}</p>
                  <a :href="data.resepsiLocation?.mapUrl" target="_blank" class="inline-flex items-center justify-center px-6 py-2.5 border border-[#d6b18a] text-[#d6b18a] rounded-lg hover:bg-[#d6b18a] hover:text-black transition-all text-xs md:text-sm uppercase tracking-wide font-medium">
                     <i class="fa-solid fa-location-dot mr-2"></i> Google Maps
                  </a>
               </div>
            </div>

            <button @click="addToCalendar" class="mt-12 bg-[#d6b18a] text-black px-8 py-3 rounded-full font-bold hover:bg-[#b48c5b] transition-all shadow-lg hover:scale-105 text-sm md:text-base">
      ...
      <section v-if="data.loveStory?.length" class="py-20 md:py-24 px-6 bg-[#1a1a1a]">
         <div class="max-w-4xl mx-auto">
            <h2 class="text-3xl md:text-4xl font-alex text-center text-[#d6b18a] mb-12 md:mb-16" v-observe>Our Journey</h2>
            
            <div class="space-y-12">
               <div v-for="(story, index) in data.loveStory" :key="index" class="flex flex-col md:flex-row gap-6 md:gap-8 items-center group" v-observe>
                  <div :class="['w-full md:w-1/2', index % 2 !== 0 ? 'md:order-2' : '']">
                     <img v-if="story.images" :src="story.images" class="w-full h-56 md:h-64 object-cover rounded-2xl shadow-lg grayscale group-hover:grayscale-0 transition-all duration-500" />
      ...
      <section v-if="galleryImages.length" class="py-20 md:py-24 px-4 bg-black">
         <h2 class="text-3xl md:text-4xl font-alex text-center text-[#d6b18a] mb-8 md:mb-12" v-observe>Captured Moments</h2>
      ...
      <section id="rsvp" class="py-20 md:py-24 px-6 relative bg-[#1a1a1a]">
         <div class="max-w-2xl mx-auto bg-black/40 backdrop-blur-xl border border-[#d6b18a]/20 rounded-3xl p-6 md:p-12 shadow-2xl" v-observe>
            <h2 class="text-2xl md:text-3xl font-serif text-center text-white mb-2">RSVP</h2>
            <p class="text-center text-gray-400 mb-8 text-xs md:text-sm">Mohon konfirmasi kehadiran Anda sebelum acara dimulai.</p>

            <form @submit.prevent="submitRSVP" class="space-y-5">
               <input v-model="rsvp.name" type="text" placeholder="Nama Lengkap" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d6b18a] transition-colors placeholder-white/30 text-sm md:text-base" required />
               
               <div class="grid grid-cols-3 gap-3">
                  <label class="flex items-center justify-center px-2 md:px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-gray-400 cursor-pointer transition-all hover:bg-white/10 text-xs md:text-base" :class="{ 'border-[#d6b18a] text-[#d6b18a] bg-[#d6b18a]/10 font-bold': rsvp.attendance === 'hadir' }">
                     <input type="radio" value="hadir" v-model="rsvp.attendance" class="hidden"> Hadir
                  </label>
                  <label class="flex items-center justify-center px-2 md:px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-gray-400 cursor-pointer transition-all hover:bg-white/10 text-xs md:text-base" :class="{ 'border-[#d6b18a] text-[#d6b18a] bg-[#d6b18a]/10 font-bold': rsvp.attendance === 'tidak' }">
                     <input type="radio" value="tidak" v-model="rsvp.attendance" class="hidden"> Maaf
                  </label>
                  <label class="flex items-center justify-center px-2 md:px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-gray-400 cursor-pointer transition-all hover:bg-white/10 text-xs md:text-base" :class="{ 'border-[#d6b18a] text-[#d6b18a] bg-[#d6b18a]/10 font-bold': rsvp.attendance === 'ragu' }">
                     <input type="radio" value="ragu" v-model="rsvp.attendance" class="hidden"> Ragu
                  </label>
               </div>

               <select v-if="rsvp.attendance === 'hadir'" v-model="rsvp.totalGuest" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d6b18a] transition-colors placeholder-white/30 text-sm md:text-base">
               ...
               <textarea v-model="rsvp.message" rows="3" placeholder="Ucapan & Doa" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#d6b18a] transition-colors placeholder-white/30 text-sm md:text-base"></textarea>

               <button type="submit" class="w-full py-3.5 bg-[#d6b18a] hover:bg-[#b48c5b] text-black font-bold rounded-xl transition-all shadow-lg mt-4 text-sm md:text-base">
                  Kirim Konfirmasi
               </button>
            </form>
         </div>
      ...
      <!-- GIFT -->
      <section v-if="data.bankAccounts?.length || data.eWalletLink?.length" class="py-20 md:py-24 px-6 bg-black text-center">
         <h2 class="text-2xl md:text-3xl font-serif text-[#d6b18a] mb-4" v-observe>Wedding Gift</h2>
         <p class="text-gray-400 mb-10 max-w-lg mx-auto text-sm md:text-base">Doa restu Anda merupakan karunia yang sangat berarti bagi kami.</p>
         
         <div class="flex flex-wrap justify-center gap-6 max-w-4xl mx-auto">
            <!-- Bank Cards -->
            <div v-for="(bank, idx) in data.bankAccounts" :key="'bank'+idx" class="bg-[#1a1a1a] border border-[#333] p-6 rounded-2xl w-full sm:w-72 hover:border-[#d6b18a]/50 transition-all" v-observe>
               ...
               <button @click="copyToClipboard(bank.accountNumber)" class="text-xs text-[#d6b18a] border border-[#d6b18a] px-4 py-1.5 rounded-full hover:bg-[#d6b18a] hover:text-black transition-colors">
