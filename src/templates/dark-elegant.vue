<template>
  <div class="relative min-h-screen bg-black overflow-hidden font-sans no-scrollbar font-montserrat text-[#f0f0f0]">
    ...
    <!-- Music Control -->
    <MusicControl v-if="data.musicChoice" :src="getMusicUrl(data.musicChoice)" :audioStart="data.audioStart" :audioEnd="data.audioEnd" primaryColor="#1a1a1a" accentColor="#d6b18a" />
      ...
      <section id="home"
        ...
        <div class="relative z-10 space-y-6" v-observe>
          ...
          <p class="text-base md:text-lg text-gray-200 font-light tracking-wide">{{
            formatDate(data.resepsiLocation?.dateTime || data.akadLocation?.dateTime) }}</p>
...
watch(() => props.data, (newVal) => {
  if (newVal && Object.keys(newVal).length > 0) {
    // Only update if it's actually a different object reference or has changed significantly
    // to avoid potential loops if initData modifies data.value
