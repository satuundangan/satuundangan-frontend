<template>
  <div class="relative min-h-screen bg-[#f0f8ff] overflow-hidden font-sans text-[#1e3a8a] selection:bg-[#ffb703] selection:text-white">
    ...
    <!-- Mobile Nav -->
    <nav v-if="!showWelcome" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-white/90 backdrop-blur-md rounded-[30px] shadow-xl border border-[#e0f2fe] w-[92%] max-w-md md:hidden transition-all duration-1000 flex overflow-x-auto no-scrollbar scroll-smooth">
      ...
      <section id="home" class="min-h-screen relative flex items-center justify-center px-6 md:px-20 py-20">
        <div class="w-full max-w-5xl grid md:grid-cols-2 gap-12 md:gap-20 items-center relative z-10" v-observe>
          <div class="order-2 md:order-1 space-y-8 text-center md:text-left">
            ...
            <p class="text-xs md:text-sm font-bold uppercase tracking-[0.4em] text-[#3b82f6]">{{ formatDate(data.resepsiLocation?.dateTime || data.akadLocation?.dateTime) }}</p>
          </div>
